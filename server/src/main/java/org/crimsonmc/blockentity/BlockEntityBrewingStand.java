...
import org.crimsonmc.inventory.BrewingInventory;
import org.crimsonmc.inventory.BrewingRecipe;
...
public class BlockEntityBrewingStand extends BlockEntitySpawnable
    ...
    public void close() {
        if (!closed) {
            for (ServerPlayer player : new HashSet<>(getInventory().getViewers())) {
    ...
    public boolean onUpdate() {
        ...
        if (canBrew) {
            ...
            for (ServerPlayer player : getInventory().getViewers()) {
