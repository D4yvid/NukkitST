...
import org.crimsonmc.exception.InvalidLoginPacketException;
...
public class LoginPacket extends DataPacket {
    ...
    public void decode() {
        ...
        try {
            str = ZlibCompression.inflate(this.get(this.getInt()), 64 * 1024 * 1024);
        } catch (Exception e) {
            return;
        }
    ...
    private void decodeChainData(Gson gson) {
        Map<String, List<String>> map = gson.fromJson(
                this.getLIntString(), new TypeToken<Map<String, List<String>>>() {
                }.getType());
    ...
    private JsonObject decodeToken(Gson gson, String token) {
        ...
        return gson.fromJson(new String(Base64.getDecoder().decode(base[1]), StandardCharsets.UTF_8),
                JsonObject.class);
    }
