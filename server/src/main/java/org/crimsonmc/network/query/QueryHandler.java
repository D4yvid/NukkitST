package org.crimsonmc.network.query;
...
public class QueryHandler {
    ...
    public QueryHandler() {
        ...
        this.server.getLogger().info(
                this.server.getLanguage().translateString("crimsonmc.server.query.start"));
        ...
        this.server.getLogger().info(this.server.getLanguage().translateString(
                "crimsonmc.server.query.info", String.valueOf(port)));

        this.regenerateToken();
        this.lastToken = this.token;
        this.regenerateInfo();
        this.server.getLogger().info(this.server.getLanguage().translateString(
                "crimsonmc.server.query.running", new String[]{addr, String.valueOf(port)}));
    }
    ...
    public static String getTokenString(String token, String salt) {
        try {
            return String.valueOf(Binary.readInt(Binary.subBytes(
                    MessageDigest.getInstance("SHA-512").digest((salt + ":" + token).getBytes()), 7, 4)));
    ...
    public void handle(String address, int port, byte[] packet) {
        ...
        switch (packetType) {
            case HANDSHAKE:
                byte[] reply =
                        Binary.appendBytes(HANDSHAKE, Binary.writeInt(sessionID),
                                getTokenString(this.token, address).getBytes(), new byte[]{0x00});
            ...
            case STATISTICS:
                ...
                if (!token.equals(getTokenString(this.token, address)) &&
                        !token.equals(getTokenString(this.lastToken, address))) {
                    break;
                }
                ...
                reply = Binary.appendBytes(STATISTICS, Binary.writeInt(sessionID),
