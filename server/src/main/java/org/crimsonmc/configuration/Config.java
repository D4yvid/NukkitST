package org.crimsonmc.configuration;
    ...
    public boolean load(String file, int type, ConfigSection defaultMap) {
        ...
        if (!this.file.exists()) {
            try {
                ...
                ServerLogger.get().error("Could not create Config " + this.file.toString(), e);
            }
        ...
        for (String line : content.split("\n")) {
            if (Pattern.compile("[a-zA-Z0-9\\-_\\.]*+=+[^\\r\\n]*").matcher(line).matches()) {
                ...
                if (this.config.containsKey(k)) {
                    ServerLogger.get().debug("[Config] Repeated property " + k + " on file " +
                            this.file.toString());
                }
                switch (v_lower) {
                    case "on":
