...
import org.crimsonmc.event.block.BlockBurnEvent;
import org.crimsonmc.event.block.BlockIgniteEvent;
import org.crimsonmc.event.entity.EntityCombustByBlockEvent;
...
public class BlockFire extends BlockFlowable {
    ...
    public void onEntityCollide(Entity entity) {
        if (!entity.hasEffect(Effect.FIRE_RESISTANCE)) {
            EntityDamageByBlockEvent ev =
                    new EntityDamageByBlockEvent(this, entity, EntityDamageEvent.CAUSE_FIRE, 1);
            entity.attack(ev);
        }
        ...
        if (!ev.isCancelled() && entity instanceof ServerPlayer && !((ServerPlayer) entity).isCreative()) {
    ...
    public int onUpdate(int type) {
        if (type == Level.BLOCK_UPDATE_NORMAL || type == Level.BLOCK_UPDATE_RANDOM) {
            if (!this.isBlockTopFacingSurfaceSolid(this.getSide(Vector3.SIDE_DOWN)) &&
                    !this.canNeighborBurn()) {
            ...
            if (!forever && this.getLevel().isRaining() &&
                ...
                } else if (!forever && !(this.getSide(Vector3.SIDE_DOWN).getBurnAbility() > 0) &&
                        meta == 15 && random.nextInt(4) == 0) {
                    this.getLevel().setBlock(this, new BlockAir(), true);
                } else {
                    int o = 0;
                        ...
                        for (int z = (int) (this.z - 1); z <= (int) (this.z + 1); ++z) {
                            for (int y = (int) (this.y - 1); y <= (int) (this.y + 4); ++y) {
                                if (x != (int) this.x || y != (int) this.y || z != (int) this.z) {
                                    ...
                                    if (chance > 0) {
                                        int t = (chance + 40 + this.getLevel().getServer().getDifficulty() * 7) /
                                                (meta + 30);
