...
import org.crimsonmc.level.sound.FizzSound;
...
public abstract class BlockLiquid extends BlockTransparent {
    ...
    public Vector3 getFlowVector() {
        ...
        if (this.getDamage() >= 8) {
            ...
            if (!this.getLevel()
                    .getBlock(this.temporalVector.setComponents(this.x, this.y, this.z - 1))
                    .canBeFlowedInto()) {
                falling = true;
                ...
                falling = true;
                    ...
                    .getBlock(this.temporalVector.setComponents(this.x - 1, this.y, this.z))
                    .canBeFlowedInto()) {
                falling = true;
            } else if (!this.getLevel()
                    .getBlock(this.temporalVector.setComponents(this.x + 1, this.y, this.z))
                ...
                falling = true;
                    ...
                    .getBlock(this.temporalVector.setComponents(this.x, this.y + 1, this.z - 1))
                    .canBeFlowedInto()) {
                falling = true;
            } else if (!this.getLevel()
                    .getBlock(this.temporalVector.setComponents(this.x, this.y + 1, this.z + 1))
                ...
                falling = true;
                    ...
                    .getBlock(this.temporalVector.setComponents(this.x - 1, this.y + 1, this.z))
                    .canBeFlowedInto()) {
                falling = true;
            } else if (!this.getLevel()
                    .getBlock(this.temporalVector.setComponents(this.x + 1, this.y + 1, this.z))
        ...
        if (type == Level.BLOCK_UPDATE_NORMAL) {
            ...
            if (decay > 0) {
                ...
                smallestFlowDecay = this.getSmallestFlowDecay(
                        this.level.getBlock(this.temporalVector.setComponents(this.x, this.y, this.z - 1)),
                        smallestFlowDecay);
                smallestFlowDecay = this.getSmallestFlowDecay(
                        this.level.getBlock(this.temporalVector.setComponents(this.x, this.y, this.z + 1)),
                        smallestFlowDecay);
                smallestFlowDecay = this.getSmallestFlowDecay(
                        this.level.getBlock(this.temporalVector.setComponents(this.x - 1, this.y, this.z)),
                        smallestFlowDecay);
                smallestFlowDecay = this.getSmallestFlowDecay(
                        this.level.getBlock(this.temporalVector.setComponents(this.x + 1, this.y, this.z)),
                        smallestFlowDecay);
                ...
                int topFlowDecay;
                if ((topFlowDecay = this.getFlowDecay(this.level.getBlock(this.level.getBlock(
                        this.temporalVector.setComponents(this.x, this.y + 1, this.z))))) >= 0) {
                    if (topFlowDecay >= 8) {
                        k = topFlowDecay;
                    } else {
                        k = topFlowDecay | 0x08;
                ...
                if (this.adjacentSources >= 2 && this instanceof BlockWater) {
                    Block bottomBlock = this.level.getBlock(
                ...
                if (this instanceof BlockLava && decay < 8 && k < 8 && k > 1 &&
            ...
            if (decay >= 0 &&
                    ((decay == 0 && this.getDamage() == 0) ||
                            (!bottomBlock.canBeFlowedInto() && !(bottomBlock instanceof BlockLiquid)))) {
    ...
    private int calculateFlowCost(Block block, int accumulatedCost, int previousDirection) {
        ...
        for (int j = 0; j < 4; ++j) {
            if ((j == 0 && previousDirection == 1) || (j == 1 && previousDirection == 0) ||
                    (j == 2 && previousDirection == 3) || (j == 3 && previousDirection == 2)) {
                double x = block.x;
                double y = block.y;
                double z = block.z;
                ...
                if (!blockSide.canBeFlowedInto() && !(blockSide instanceof BlockLiquid)) {
                    ...
                    continue;
                        ...
                        .getBlock(this.temporalVector.setComponents(x, y - 1, z))
                        .canBeFlowedInto()) {
    ...
    protected void triggerLavaMixEffects(Vector3 pos) {
        this.getLevel().addSound(new FizzSound(
                pos.add(0.5, 0.5, 0.5),
                2.6F + (ThreadLocalRandom.current().nextFloat() - ThreadLocalRandom.current().nextFloat()) *
                        0.8F));
