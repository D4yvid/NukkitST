...
public class HelpCommand extends VanillaCommand {
    ...
    public HelpCommand(String name) {
        super(name, "%crimsonmc.command.help.description", "%commands.help.usage", new String[]{"?"});
        this.setPermission("crimsonmc.command.help");
    }
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if (command.equals("")) {
            ...
            int totalPage = commands.size() % pageHeight == 0 ? commands.size() / pageHeight
                    : commands.size() / pageHeight + 1;
            ...
            sender.sendMessage(new TranslationContainer(
                    ...
                    new String[]{String.valueOf(pageNumber), String.valueOf(totalPage)}));
            int i = 1;
            for (Command command1 : commands.values()) {
                if (i >= (pageNumber - 1) * pageHeight + 1 &&
                        i <= Math.min(commands.size(), pageNumber * pageHeight)) {
                    sender.sendMessage(TextFormat.DARK_GREEN + "/" + command1.getName() + ": " +
                            TextFormat.WHITE + command1.getDescription());
                }
                i++;
            }
            ...
            if (cmd != null) {
                if (cmd.testPermissionSilent(sender)) {
                    String message = TextFormat.YELLOW + "--------- " + TextFormat.WHITE + " Help: /" +
                            cmd.getName() + TextFormat.YELLOW + " ---------\n";
                    message +=
                            TextFormat.GOLD + "Description: " + TextFormat.WHITE + cmd.getDescription() + "\n";
                    String usage = "";
