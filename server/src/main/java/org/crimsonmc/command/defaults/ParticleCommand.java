...
import org.crimsonmc.level.particle.*;
...
public class ParticleCommand extends VanillaCommand {
    ...
    public ParticleCommand(String name) {
        super(name, "%crimsonmc.command.particle.description", "%crimsonmc.command.particle.usage");
        this.setPermission("crimsonmc.command.particle");
    }
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        for (int i = 0; i < count; i++) {
            particle.setComponents(pos.x + (random.nextFloat() * 2 - 1) * xd,
    ...
    private Particle getParticle(String name, Vector3 pos, float xd, float yd, float zd,
                                 Integer data) {
        switch (name) {
            case "explode":
        ...
        if (name.startsWith("iconcrack_")) {
            ...
            String[] d = name.split("_");
            if (d.length == 2) {
                return new TerrainParticle(
                        pos, Block.get(Integer.valueOf(d[1]) & 0xff, Integer.valueOf(d[1]) >> 12));
            }
        } else if (name.startsWith("blockdust_")) {
            String[] d = name.split("_");
            if (d.length >= 4) {
                return new DustParticle(pos, Integer.valueOf(d[1]) & 0xff, Integer.valueOf(d[2]) & 0xff,
                        Integer.valueOf(d[3]) & 0xff,
                        d.length >= 5 ? Integer.valueOf(d[4]) & 0xff : 255);
            }
        }
