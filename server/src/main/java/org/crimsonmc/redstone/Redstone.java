...
import org.crimsonmc.block.BlockSolid;
...
public class Redstone {
    ...
    // I set it to 16 in order to calculate the energy in blocks, such as the redstone torch under the
    // cobblestone. At that time, the cobblestone's energy is 16, not 15. If you put a redstone wire
    // next to it, the redstone wire will got 15 energy. So, POWER_WEAKEST also means that energy in
    // blocks, not redstone wire it self. So set it to 1.
    ...
    private static void addToQueue(Queue<UpdateObject> updateQueue, Block location) {
        ...
        for (int side : new int[]{Vector3.SIDE_NORTH, Vector3.SIDE_SOUTH, Vector3.SIDE_EAST,
                Vector3.SIDE_WEST, Vector3.SIDE_UP, Vector3.SIDE_DOWN}) {
        ...
        if (location instanceof BlockRedstoneWire) {
            ...
            for (int side : new int[]{Vector3.SIDE_NORTH, Vector3.SIDE_WEST, Vector3.SIDE_EAST,
                    Vector3.SIDE_SOUTH}) {
    ...
    private static void addToDeactiveQueue(Queue<UpdateObject> updateQueue, Block location,
                                           Map<String, Block> closedMap,
                                           Queue<UpdateObject> sourceList, int updateLevel) {
        if (updateLevel < 0) {
            return;
        }
        for (int side : new int[]{Vector3.SIDE_NORTH, Vector3.SIDE_SOUTH, Vector3.SIDE_EAST,
                Vector3.SIDE_WEST, Vector3.SIDE_UP, Vector3.SIDE_DOWN}) {
            if (location.getSide(side).isPowerSource() ||
                    (updateLevel == 0 && location.getSide(side).isPowered())) {
