...
import org.crimsonmc.item.ItemBookEnchanted;
import org.crimsonmc.item.ItemDye;
import org.crimsonmc.item.enchantment.Enchantment;
import org.crimsonmc.item.enchantment.EnchantmentEntry;
import org.crimsonmc.item.enchantment.EnchantmentList;
...
public class EnchantInventory extends ContainerInventory {
    ...
    public void onOpen(ServerPlayer who) {
        ...
        if (this.levels == null) {
            ...
            double base = (double) random.nextRange(1, 8) + (bookshelfAmount / 2d) +
                    (double) random.nextRange(0, bookshelfAmount);
            ...
            if (item.getId() == Item.AIR) {
                ...
                if (this.entries == null) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        k += ThreadLocalRandom.current().nextInt(0, Math.round(enchantAbility / 2f));
                        k++;
                        float bonus = (ThreadLocalRandom.current().nextFloat() +
                        ...
                        for (Enchantment enchantment : Enchantment.getEnchantments()) {
                            if (enchantment.canEnchant(item)) {
                                for (int enchLevel = enchantment.getMinLevel();
                                     enchLevel < enchantment.getMaxLevel(); enchLevel++) {
                                    if (modifiedLevel >= enchantment.getMinEnchantAbility(enchLevel) &&
                                            modifiedLevel <= enchantment.getMaxEnchantAbility(enchLevel)) {
                        ...
                        this.entries[i] = new EnchantmentEntry(result.stream().toArray(Enchantment[]::new),
                                level, Enchantment.getRandomName());
                    }
                }
    ...
    public void onEnchant(ServerPlayer who, Item before, Item after) {
        ...
        if (!before.hasEnchantments() && after.hasEnchantments() && after.getId() == result.getId() &&
            ...
            for (int i = 0; i < 3; i++) {
                if (Arrays.equals(enchantments, this.entries[i].getEnchantments())) {
                    ...
                    if (lapis.getId() == Item.DYE && lapis.getDamage() == ItemDye.DyeColor.BLUE.getDyeData() &&
                            lapis.getCount() > i && level >= cost) {
