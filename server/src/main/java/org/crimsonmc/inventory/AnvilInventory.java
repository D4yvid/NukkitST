...
import org.crimsonmc.level.sound.AnvilUseSound;
...
public class AnvilInventory extends ContainerInventory {
    ...
    public boolean onRename(ServerPlayer player, Item resultItem) {
        ...
        if (local.getId() != 0 && second.getId() == 0) { // only rename
            ...
            if (local.getId() != 0 && second.getId() != 0) {
                ...
                ArrayList<Enchantment> enchantments =
                        new ArrayList<>(Arrays.asList(second.getEnchantments()));
    ...
    public void onClose(ServerPlayer who) {
        super.onClose(who);
        who.craftingType = ServerPlayer.CRAFTING_SMALL;
    ...
    public void onOpen(ServerPlayer who) {
        super.onOpen(who);
        who.craftingType = ServerPlayer.CRAFTING_ANVIL;
    }
}