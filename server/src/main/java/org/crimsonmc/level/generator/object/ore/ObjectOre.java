...
import org.crimsonmc.math.VectorMath;
...
public class ObjectOre {
    ...
    private final RandomUtilities random;

    public ObjectOre(RandomUtilities random, OreType type) {
        this.type = type;
        this.random = random;
    }
    ...
    public void placeObject(ChunkManager level, int x, int y, int z) {
        ...
        for (int count = 0; count <= clusterSize; ++count) {
            ...
            double size = ((Math.sin(count * (Math.PI / clusterSize)) + 1) * this.random.nextFloat() *
                    clusterSize / 16 +
                    1) /
                    2;
