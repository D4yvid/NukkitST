package org.crimsonmc.level.generator;
...
import org.crimsonmc.level.generator.biome.BiomeSelector;
import org.crimsonmc.level.generator.noise.Simplex;
import org.crimsonmc.level.generator.object.ore.OreType;
import org.crimsonmc.level.generator.populator.Populator;
import org.crimsonmc.level.generator.populator.PopulatorCaves;
import org.crimsonmc.level.generator.populator.PopulatorGroundCover;
import org.crimsonmc.level.generator.populator.PopulatorOre;
...
public class Normal extends Generator {
    ...
    private RandomUtilities random;
    ...
    @Override
    public void init(ChunkManager level, RandomUtilities random) {
        this.level = level;
        this.random = random;
        ...
        ores.setOreTypes(new OreType[]{new OreType(new BlockOreCoal(), 20, 16, 0, 128),
    ...
    public void generateChunk(int chunkX, int chunkZ) {
        ...
        double[][] seaFloorNoise =
                Generator.getFastNoise2D(this.noiseSeaFloor, 16, 16, 4, chunkX * 16, 0, chunkZ * 16);
        double[][] landNoise =
                Generator.getFastNoise2D(this.noiseLand, 16, 16, 4, chunkX * 16, 0, chunkZ * 16);
        double[][] mountainNoise =
                Generator.getFastNoise2D(this.noiseMountains, 16, 16, 4, chunkX * 16, 0, chunkZ * 16);
        double[][] baseNoise =
                Generator.getFastNoise2D(this.noiseBaseGround, 16, 16, 4, chunkX * 16, 0, chunkZ * 16);
        double[][] riverNoise =
                Generator.getFastNoise2D(this.noiseRiver, 16, 16, 4, chunkX * 16, 0, chunkZ * 16);
        ...
        for (int genx = 0; genx < 16; genx++) {
            for (int genz = 0; genz < 16; genz++) {
                    ...
                    if (riverGenerate > -0.25F && riverGenerate < 0.25F) {
                        ...
                        if (genyHeight < seaHeight) {
                            ...
                            if (genyHeight <= seaHeight - 8) {
                                int genyHeight1 =
                                        seaHeight - 9 + (int) (basegroundHeight * (baseNoise[genx][genz] + 1F));
                ...
                chunk.setBiomeColor(genx, genz, (biomecolor >> 16), (biomecolor >> 8) & 0xff,
                        (biomecolor & 0xff));
                // generating
                ...
                for (int geny = 0; geny <= generateHeight; geny++) {
                    if (geny <= bedrockDepth && (geny == 0 || random.nextRange(1, 5) == 1)) {
                        ...
                        if ((biome.getId() == Biome.ICE_PLAINS || biome.getId() == Biome.TAIGA) &&
                                geny == seaHeight) {
