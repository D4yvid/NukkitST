...
import org.crimsonmc.level.generator.noise.Noise;
...
public abstract class Generator {
    ...
    public static double[] getFastNoise1D(Noise noise, int xSize, int samplingRate, int x, int y,
                                          int z) {
        if (samplingRate == 0) {
        ...
        for (int xx = 0; xx < xSize; ++xx) {
            if (xx % samplingRate != 0) {
                ...
                noiseArray[nx] = Noise.linearLerp(xx, nx, nx + samplingRate, noiseArray[nx],
                        noiseArray[nx + samplingRate]);
            }
        }
    ...
    public static double[][] getFastNoise2D(Noise noise, int xSize, int zSize, int samplingRate,
                                            int x, int y, int z, int xZoom, int zZoom) {
        if (samplingRate == 0) {
            throw new IllegalArgumentException("samplingRate cannot be 0");
        }
        if (xSize % samplingRate != 0) {
    ...
    public static double[][][] getFastNoise3D(Noise noise, int xSize, int ySize, int zSize,
                                              int xSamplingRate, int ySamplingRate, int zSamplingRate,
                                              int x, int y, int z) {
        if (xSamplingRate == 0) {
    ...
    public abstract void init(ChunkManager level, RandomUtilities random);
