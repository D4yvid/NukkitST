...
import org.crimsonmc.event.block.BlockBreakEvent;
import org.crimsonmc.event.block.BlockPlaceEvent;
import org.crimsonmc.event.block.BlockUpdateEvent;
import org.crimsonmc.event.level.*;
import org.crimsonmc.event.player.PlayerInteractEvent;
import org.crimsonmc.event.redstone.RedstoneUpdateEvent;
import org.crimsonmc.event.weather.LightningStrikeEvent;
...
import org.crimsonmc.level.format.Chunk;
...
import org.crimsonmc.level.generator.task.*;
import org.crimsonmc.level.particle.DestroyBlockParticle;
import org.crimsonmc.level.particle.Particle;
import org.crimsonmc.level.sound.Sound;
import org.crimsonmc.logger.ServerLogger;
import org.crimsonmc.math.*;
import org.crimsonmc.metadata.BlockMetadataStore;
...
import org.crimsonmc.util.PriorityObject;
...
public class Level implements ChunkManager, Metadatable {
    ...
    private final Map<Long, ServerPlayer> players = new HashMap<>();
    ...
    private final Map<Long, Map<Integer, ServerPlayer>> playerLoaders = new HashMap<>();
    ...
    private final Map<Long, Map<Integer, ServerPlayer>> chunkSendQueue = new HashMap<>();
    ...
    private final HashMap<Integer, Class<? extends Block>> randomTickBlocks =
            new HashMap<Integer, Class<? extends Block>>() {
                {
    ...
    public Level(Server server, String name, String path, Class<? extends LevelProvider> provider) {
        ...
        this.server.getLogger().info(this.server.getLanguage().translateString(
                "crimsonmc.level.preparing", TextFormat.GREEN + this.provider.getName() + TextFormat.WHITE));
        ...
        this.chunkTickRadius =
                Math.min(this.server.getViewDistance(),
        ...
        this.chunkPopulationQueueSize =
                (int) this.server.getConfig("chunk-generation.population-queue-size", 2);
    ...
    public static int generateChunkLoaderId(ChunkLoader loader) {
        if (loader.getLoaderId() == null || loader.getLoaderId() == 0) {
            ...
            throw new IllegalStateException("ChunkLoader has a loader id already assigned: " +
                    loader.getLoaderId());
        }
    }
    ...
    public void initLevel() {
        try {
            this.generatorInstance =
                    this.generator.getConstructor(Map.class).newInstance(this.provider.getGeneratorOptions());
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        this.generatorInstance.init(this, new RandomUtilities(this.getSeed()));
    ...
    public void registerGenerator() {
        ...
        for (int i = 0; i < size; ++i) {
            this.server.getScheduler().scheduleAsyncTask(
                    new GeneratorRegisterTask(this, this.generatorInstance));
        }
    }
    ...
    public void addSound(Sound sound) {
        this.addSound(sound, (ServerPlayer[]) null);
    }

    public void addSound(Sound sound, ServerPlayer player) {
        this.addSound(sound, new ServerPlayer[]{player});
    }

    public void addSound(Sound sound, ServerPlayer[] players) {
    ...
    public void addSound(Sound sound, Collection<ServerPlayer> players) {
        this.addSound(sound, players.stream().toArray(ServerPlayer[]::new));
    }

    public void addParticle(Particle particle) {
        this.addParticle(particle, (ServerPlayer[]) null);
    }

    public void addParticle(Particle particle, ServerPlayer player) {
        this.addParticle(particle, new ServerPlayer[]{player});
    }

    public void addParticle(Particle particle, ServerPlayer[] players) {
    ...
    public void addParticle(Particle particle, Collection<ServerPlayer> players) {
        this.addParticle(particle, players.stream().toArray(ServerPlayer[]::new));
    }
    ...
    public boolean unload(boolean force) {
        ...
        this.server.getLogger().info(this.server.getLanguage().translateString(
                "crimsonmc.level.unloading", TextFormat.GREEN + this.getName() + TextFormat.WHITE));
        Level defaultLevel = this.server.getDefaultLevel();

        for (ServerPlayer player : new ArrayList<>(this.getPlayers().values())) {
    ...
    public Map<Integer, ServerPlayer> getChunkPlayers(int chunkX, int chunkZ) {
    ...
    public void registerChunkLoader(ChunkLoader loader, int chunkX, int chunkZ, boolean autoLoad) {
        ...
        if (loader instanceof ServerPlayer) {
            this.playerLoaders.get(index).put(hash, (ServerPlayer) loader);
        }
    ...
    public void sendTime(ServerPlayer player) {
        if (this.stopTime) {
    ...
    public void sendTime() {
        if (this.stopTime) {
            ...
            pk0.time = (int) this.time;
            pk0.started = true;
            Server.broadcastPacket(this.players.values().stream().toArray(ServerPlayer[]::new), pk0);
        }
        ...
        Server.broadcastPacket(this.players.values().stream().toArray(ServerPlayer[]::new), pk);
    }
    ...
    public void doTick(int currentTick) {
            ...
            if (!this.players.isEmpty()) {
                for (Map.Entry<Long, SoftReference<Map<Short, Object>>> entry : changedBlocks.entrySet()) {
                    ...
                    if (blocks == null || blocks.size() > MAX_BLOCK_CACHE) {
                        ...
                        for (ServerPlayer p : this.getChunkPlayers(chunkX, chunkZ).values()) {
                            p.onChunkChanged(chunk);
                        }
                    } else {
                        Collection<ServerPlayer> toSend = this.getChunkPlayers(chunkX, chunkZ).values();
                        ServerPlayer[] playerArray = toSend.toArray(new ServerPlayer[toSend.size()]);
        ...
        for (long index : this.motionToSend.keySet()) {
            ...
            pk.entities = this.motionToSend.get(index).values().stream().toArray(
                    SetEntityMotionPacket.Entry[]::new);
        ...
        for (long index : this.chunkPackets.keySet()) {
            int chunkX = Level.getHashX(index);
            int chunkZ = Level.getHashZ(index);
            ServerPlayer[] chunkPlayers =
                    this.getChunkPlayers(chunkX, chunkZ).values().stream().toArray(ServerPlayer[]::new);
    ...
    public void checkSleep() {
        ...
        for (ServerPlayer p : this.getPlayers().values()) {
            if (!p.isSleeping()) {
                resetTime = false;
                break;
            }
    ...
    public void sendBlockExtraData(int x, int y, int z, int id, int data, ServerPlayer[] players) {
    ...
    public void sendBlocks(ServerPlayer[] target, Vector3[] blocks) {
        this.sendBlocks(target, blocks, UpdateBlockPacket.FLAG_NONE);
    }

    public void sendBlocks(ServerPlayer[] target, Vector3[] blocks, int flags) {
        this.sendBlocks(target, blocks, flags, false);
    }

    public void sendBlocks(ServerPlayer[] target, Vector3[] blocks, int flags, boolean optimizeRebuilds) {
        ...
        if (optimizeRebuilds) {
            ...
            for (Vector3 b : blocks) {
                ...
                if (b instanceof Block) {
                    ...
                    list.add(new UpdateBlockPacket.Entry((int) b.x, (int) b.z, (int) b.y, ((Block) b).getId(),
                            ((Block) b).getDamage(),
    ...
    private void tickChunks() {
        ...
        int chunksPerLoader =
                Math.min(200, Math.max(1, (int) (((double) (this.chunksPerTicks - this.loaders.size()) /
                        this.loaders.size() +
                        0.5))));
        ...
        for (ChunkLoader loader : this.loaders.values()) {
            ...
            Long index = Level.chunkHash(chunkX, chunkZ);
            int existingLoaders =
                    Math.max(0, this.chunkTickList.containsKey(index) ? this.chunkTickList.get(index) : 0);
        ...
        for (Long index : new ArrayList<>(this.chunkTickList.keySet())) {
            ...
            if (!this.chunks.containsKey(index) ||
                    (chunk = this.getChunk(chunkX, chunkZ, false)) == null) {
                    ...
                    if (!(section instanceof EmptyChunkSection)) {
                        ...
                        for (int i = 0; i < 3; ++i, k >>= 10) {
                            ...
                            if (this.randomTickBlocks.containsKey(blockId)) {
                                ...
                                try {
                                    Block block =
                                            clazz.getConstructor(int.class).newInstance(section.getBlockData(x, y, z));
                                    block.x = chunkX * 16 + x;
                                    block.y = (Y << 4) + y;
                                    block.z = chunkZ * 16 + z;
                                    block.level = this;
                ...
                for (int Y = 0; Y < 8 && (Y < 3 || blockTest != 0); ++Y) {
                    ...
                    for (int i = 0; i < 3; ++i, k >>= 10) {
                        ...
                        if (this.randomTickBlocks.containsKey(blockId)) {
                            ...
                            Block block;
                            try {
                                block = clazz.getConstructor(int.class).newInstance(
                                        chunk.getBlockData(x, y + (Y << 4), z));
                            } catch (Exception e) {
                                throw new RuntimeException(e);
                            }
                            block.x = chunkX * 16 + x;
    ...
    public void updateAroundRedstone(Block block) {
        ...
        this.server.getPluginManager().callEvent(ev);
        if (!ev.isCancelled()) {
            for (Entity entity : this.getNearbyEntities(new AxisAlignedBB(
                    block.x - 1, block.y - 1, block.z - 1, block.x + 1, block.y + 1, block.z + 1))) {
                entity.scheduleUpdate();
            }
            ev.getBlock().onUpdate(BLOCK_UPDATE_NORMAL);
    ...
    public void scheduleUpdate(Vector3 pos, int delay) {
        ...
        this.updateQueue.add(new PriorityObject(new Vector3((int) pos.x, (int) pos.y, (int) pos.z),
                delay + this.server.getTick()));
    }
    ...
    public int getFullLight(Vector3 pos) {
        ...
        if (chunk != null) {
            ...
            if (level < 15) {
                level = Math.max(
                        chunk.getBlockLight((int) pos.x & 0x0f, (int) pos.y & 0x7f, (int) pos.z & 0x0f), level);
                // todo: check this
            }
        }
    ...
    public Block getBlock(Vector3 pos, boolean cached) {
        ...
        if (cached && this.blockCache.containsKey(index)) {
            ...
            fullState = this.chunks.get(chunkIndex)
                    .getFullBlock((int) pos.x & 0x0f, (int) pos.y & 0x7f, (int) pos.z & 0x0f);
        } else {
            fullState = 0;
        }
    ...
    public void updateBlockLight(int x, int y, int z) {
        ...
        while (!lightRemovalQueue.isEmpty()) {
            ...
            this.computeRemoveBlockLight((int) node.x - 1, (int) node.y, (int) node.z, lightLevel,
                    lightRemovalQueue, lightPropagationQueue, removalVisited,
                    visited);
            this.computeRemoveBlockLight((int) node.x + 1, (int) node.y, (int) node.z, lightLevel,
                    lightRemovalQueue, lightPropagationQueue, removalVisited,
                    visited);
            this.computeRemoveBlockLight((int) node.x, (int) node.y - 1, (int) node.z, lightLevel,
                    lightRemovalQueue, lightPropagationQueue, removalVisited,
                    visited);
            this.computeRemoveBlockLight((int) node.x, (int) node.y + 1, (int) node.z, lightLevel,
                    lightRemovalQueue, lightPropagationQueue, removalVisited,
                    visited);
            this.computeRemoveBlockLight((int) node.x, (int) node.y, (int) node.z - 1, lightLevel,
                    lightRemovalQueue, lightPropagationQueue, removalVisited,
                    visited);
            this.computeRemoveBlockLight((int) node.x, (int) node.y, (int) node.z + 1, lightLevel,
                    lightRemovalQueue, lightPropagationQueue, removalVisited,
    ...
    public boolean setBlock(Vector3 pos, Block block, boolean direct, boolean update) {
        ...
        if (this.getChunk((int) pos.x >> 4, (int) pos.z >> 4, true)
                .setBlock((int) pos.x & 0x0f, (int) pos.y & 0x7f, (int) pos.z & 0x0f, block.getId(),
                        block.getDamage())) {
            Position position;
            ...
            if (direct) {
                this.sendBlocks(this.getChunkPlayers((int) position.x >> 4, (int) position.z >> 4)
                                .values()
                                .stream()
                                .toArray(ServerPlayer[]::new),
                        new Block[]{block}, UpdateBlockPacket.FLAG_ALL_PRIORITY);
                this.chunkCache.remove(index);
            } else {
    ...
    public Item useBreakOn(Vector3 vector, Item item, ServerPlayer player) {
        return this.useBreakOn(vector, item, player, false);
    }

    public Item useBreakOn(Vector3 vector, Item item, ServerPlayer player, boolean createParticles) {
        ...
        if (player != null) {
            ...
            BlockBreakEvent ev =
                    new BlockBreakEvent(player, target, item, player.isCreative(),
        ...
        if (tag instanceof ListTag) {
            ...
            for (Tag v : ((ListTag<Tag>) tag).getAll()) {
                if (v instanceof StringTag) {
                    ...
                    if (entry.getId() > 0 && entry.getBlock() != null &&
        ...
        if (createParticles) {
            Map<Integer, ServerPlayer> players = this.getChunkPlayers((int) target.x >> 4, (int) target.z >> 4);
        ...
        if (blockEntity != null) {
            if (blockEntity instanceof InventoryHolder) {
                ...
                for (Item chestItem :
                        ((InventoryHolder) blockEntity).getInventory().getContents().values()) {
    ...
    public void dropExpOrb(Vector3 source, int exp, Vector3 motion, int delay) {
        ...
        CompoundTag nbt = new CompoundTag()
                ...
                .putList(new ListTag<FloatTag>("Rotation")
                        ...
                        .add(new FloatTag("", 0)));
        Entity entity =
                new EntityXPOrb(this.getChunk(source.getFloorX() >> 4, source.getFloorZ() >> 4), nbt);
    ...
    public Item useItemOn(Vector3 vector, Item item, int face, float fx, float fy, float fz,
        ...
        if (player != null) {
            PlayerInteractEvent ev =
                    new PlayerInteractEvent(player, item, target, face,
                            target.getId() == 0 ? PlayerInteractEvent.RIGHT_CLICK_AIR
        ...
        if (hand.isSolid() && hand.getBoundingBox() != null) {
            ...
            for (Entity e : entities) {
                if (e instanceof EntityArrow || e instanceof EntityItem ||
                        (e instanceof ServerPlayer && ((ServerPlayer) e).isSpectator())) {
                    continue;
                }
                ++realCount;
            }
    ...
    public Entity[] getCollidingEntities(AxisAlignedBB bb, Entity entity) {
        ...
        if (entity == null || entity.canCollide()) {
            int minX = MathUtilities.floorDouble((bb.minX - 2) / 16);
            int maxX = MathUtilities.ceilDouble((bb.maxX + 2) / 16);
            int minZ = MathUtilities.floorDouble((bb.minZ - 2) / 16);
            int maxZ = MathUtilities.ceilDouble((bb.maxZ + 2) / 16);
    ...
    public BlockEntity getBlockEntityById(long blockEntityId) {
        return this.blockEntities.containsKey(blockEntityId) ? this.blockEntities.get(blockEntityId)
                : null;
    }
    ...
    public void setBlockExtraDataat(int x, int y, int z, int id, int data) {
        this.getChunk(x >> 4, z >> 4, true)
                .setBlockExtraData(x & 0x0f, y & 0x7f, z & 0x0f, (data << 8) | id);
    ...
    public void setBlockSkyLightAt(int x, int y, int z, int level) {
        this.getChunk(x >> 4, z >> 4, true)
                .setBlockSkyLight(x & 0x0f, y & 0x7f, z & 0x0f, level & 0x0f);
    }
    ...
    public void generateChunkCallback(int x, int z, BaseFullChunk chunk) {
        ...
        if (this.chunkPopulationQueue.containsKey(index)) {
            ...
            if (chunk != null && (oldChunk == null || !oldChunk.isPopulated()) && chunk.isPopulated() &&
                ...
                this.chunkPopulationLock.containsKey(index)) {
    ...
    public void setChunk(int chunkX, int chunkZ, BaseFullChunk chunk, boolean unload) {
        ...
        if (unload && oldChunk != null) {
            ...
            Map<Long, BlockEntity> oldBlockEntities =
                    oldChunk != null ? oldChunk.getBlockEntities() : new HashMap<>();
    ...
    public BlockColor getMapColorAt(int x, int z) {
        ...
        while (y > 1) {
            ...
            BlockColor blockMapColor = block.getColor();
            if (blockMapColor.getAlpha() == 0x00) {
                y--;
            } else {
    ...
    public void requestChunk(int x, int z, ServerPlayer player) {
        Long index = Level.chunkHash(x, z);
    ...
    private void sendChunkFromCache(int x, int z) {
        ...
        if (this.chunkSendTasks.containsKey(index)) {
            for (ServerPlayer player : this.chunkSendQueue.get(index).values()) {
    ...
    public void chunkRequestCallback(int x, int z, byte[] payload, byte ordering) {
        ...
        if (this.cacheChunks && !this.chunkCache.containsKey(index)) {
            this.chunkCache.put(index, ServerPlayer.getChunkCacheFromData(x, z, payload, ordering));
            this.sendChunkFromCache(x, z);
            return;
        }
    ...
    public void addEntity(Entity entity) {
        ...
        if (entity instanceof ServerPlayer) {
            this.players.put(entity.getId(), (ServerPlayer) entity);
        }
    ...
    public boolean unloadChunk(int x, int z, boolean safe, boolean trySave) {
        ...
        try {
            ...
            logger.error(
                    this.server.getLanguage().translateString("crimsonmc.level.chunkUnloadError", e.toString()));
            logger.exception(e);
        }
    ...
    public void addEntityMovement(int chunkX, int chunkZ, long entityId, double x, double y, double z,
    ...
    public void addPlayerMovement(int chunkX, int chunkZ, long entityId, double x, double y, double z,
    ...
    public void sendWeather(ServerPlayer[] players) {
        if (players == null) {
            players = this.getPlayers().values().stream().toArray(ServerPlayer[]::new);
        }
    ...
    public void sendWeather(ServerPlayer player) {
        if (player != null) {
            this.sendWeather(new ServerPlayer[]{player});
        }
    }

    public void sendWeather(Collection<ServerPlayer> players) {
        if (players == null) {
            players = this.getPlayers().values();
        }
        this.sendWeather(players.stream().toArray(ServerPlayer[]::new));
    }
