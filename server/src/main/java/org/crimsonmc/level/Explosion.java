            ...
            for (int j = 0; j < this.rays; ++j) {
                for (int k = 0; k < this.rays; ++k) {
                    if (i == 0 || i == mRays || j == 0 || j == mRays || k == 0 || k == mRays) {
                        vector.setComponents((double) i / (double) mRays * 2d - 1,
                                (double) j / (double) mRays * 2d - 1,
                                (double) k / (double) mRays * 2d - 1);
                        double len = vector.length();
                        vector.setComponents((vector.x / len) * this.stepLen, (vector.y / len) * this.stepLen,
                                (vector.z / len) * this.stepLen);
                        double pointerX = this.source.x;
                        double pointerY = this.source.y;
                        double pointerZ = this.source.z;

                        for (double blastForce =
                             this.size * (ThreadLocalRandom.current().nextInt(700, 1301)) / 1000d;
                             blastForce > 0; blastForce -= this.stepLen * 0.75d) {
                            int x = (int) pointerX;
                            int y = (int) pointerY;
                            int z = (int) pointerZ;
    ...
    public boolean explodeB() {
        ...
        if (this.what instanceof Entity) {
            EntityExplodeEvent ev =
                    new EntityExplodeEvent((Entity) this.what, this.source, this.affectedBlocks, yield);
        ...
        double minX = MathUtilities.floorDouble(this.source.x - explosionSize - 1);
        double maxX = MathUtilities.ceilDouble(this.source.x + explosionSize + 1);
        double minY = MathUtilities.floorDouble(this.source.y - explosionSize - 1);
        double maxY = MathUtilities.ceilDouble(this.source.y + explosionSize + 1);
        double minZ = MathUtilities.floorDouble(this.source.z - explosionSize - 1);
        double maxZ = MathUtilities.ceilDouble(this.source.z + explosionSize + 1);

        AxisAlignedBB explosionBB = new AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ);

        Entity[] list = this.level.getNearbyEntities(
                explosionBB, this.what instanceof Entity ? (Entity) this.what : null);
            ...
            if (distance <= 1) {
                ...
                if (this.what instanceof Entity) {
                    EntityDamageByEntityEvent ev = new EntityDamageByEntityEvent(
                            (Entity) this.what, entity, EntityDamageEvent.CAUSE_ENTITY_EXPLOSION, damage);
                    entity.attack(ev);
                } else if (this.what instanceof Block) {
                    EntityDamageByBlockEvent ev = new EntityDamageByBlockEvent(
                            (Block) this.what, entity, EntityDamageEvent.CAUSE_BLOCK_EXPLOSION, damage);
                    entity.attack(ev);
                } else {
