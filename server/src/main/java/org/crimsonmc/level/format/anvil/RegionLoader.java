...
import org.crimsonmc.level.format.generic.BaseRegionLoader;
...
public class RegionLoader extends BaseRegionLoader {
    ...
    public Chunk readChunk(int x, int z) throws IOException {
        ...
        if (length <= 0 || length >= MAX_SECTOR_LENGTH) {
            if (length >= MAX_SECTOR_LENGTH) {
                ...
                ServerLogger.get().error("Corrupted chunk header detected");
            }
            return null;
        }

        if (length > (table[1] << 12)) {
            ServerLogger.get().error("Corrupted bigger chunk detected");
            table[1] = length >> 12;
            this.locationTable.put(index, table);
            this.writeLocationIndex(index);
        } else if (compression != COMPRESSION_ZLIB && compression != COMPRESSION_GZIP) {
            ServerLogger.get().error("Invalid compression type");
            return null;
        }
        ...
        if (chunk != null) {
            ...
            ServerLogger.get().error("Corrupted chunk detected");
            return null;
        }
    }
    ...
    public int doSlowCleanUp() throws Exception {
        for (int i = 0; i < 1024; i++) {
            ...
            try {
                chunk = ZlibCompression.inflate(Arrays.copyOf(chunk, 5));
            } catch (Exception e) {
                this.locationTable.put(i, new Integer[]{0, 0, 0});
                continue;
            }
            chunk = ZlibCompression.deflate(chunk, 9);
