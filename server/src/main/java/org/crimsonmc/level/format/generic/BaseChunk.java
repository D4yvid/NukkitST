package org.crimsonmc.level.format.generic;
...
public abstract class BaseChunk extends BaseFullChunk implements Chunk {
    ...
    public boolean setBlock(int x, int y, int z, Integer blockId, Integer meta) {
        ...
        try {
            ...
            int Y = y >> 4;
            try {
                this.setInternalSection(
                        Y, (ChunkSection) this.providerClass.getMethod("createChunkSection", int.class)
                                .invoke(this.providerClass, Y));
            } catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException e1) {
                Server.getInstance().getLogger().exception(e1);
            }
            return this.sections[y >> 4].setBlock(x, y & 0x0f, z, id & 0xff, damage & 0x0f);
        }
    }
    ...
    public boolean setSection(float fY, ChunkSection section) {
        ...
        if (Arrays.equals(emptyIdArray, section.getIdArray()) &&
                Arrays.equals(emptyDataArray, section.getDataArray())) {
    ...
    public boolean load(boolean generate) throws IOException {
        return this.getProvider() != null &&
                this.getProvider().getChunk(this.getX(), this.getZ(), true) != null;
    }
