...
public class QueryRegenerateEvent extends ServerEvent {
    ...
    public QueryRegenerateEvent(Server server, int timeout) {
        ...
        for (ServerPlayer player : server.getOnlinePlayers().values()) {
            if (player.isOnline()) {
                players.add(player);
            }
        }
        this.players = players.toArray(new ServerPlayer[players.size()]);
    ...
    public ServerPlayer[] getPlayerList() {
        return players;
    }

    public void setPlayerList(ServerPlayer[] players) {
