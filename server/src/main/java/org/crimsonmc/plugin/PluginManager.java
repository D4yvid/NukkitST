...
import org.crimsonmc.command.PluginCommand;
import org.crimsonmc.command.SimpleCommandMap;
import org.crimsonmc.event.*;
...
public class PluginManager {
    ...
    public boolean registerInterface(Class<? extends PluginLoader> loaderClass) {
        if (loaderClass != null) {
            try {
                ...
                this.fileAssociations.put(loaderClass.getName(),
                        ...
                        if (description != null) {
                            ...
                            for (String version : description.getCompatibleAPIs()) {
                                ...
                                if (!Pattern.matches("[0-9]\\.[0-9]\\.[0-9]", version)) {
                                    this.server.getLogger().error(this.server.getLanguage().translateString(
                                            "crimsonmc.plugin.loadError", new String[]{name, "Wrong API format"}));
                                    continue;
                                }
                                ...
                                if (!Objects.equals(Integer.valueOf(versionArray[0]),
                                        Integer.valueOf(apiVersion[0]))) {
                                    continue;
                                }
                            ...
                            if (!compatible) {
                                this.server.getLogger().error(this.server.getLanguage().translateString(
                                        "crimsonmc.plugin.loadError",
                                        new String[]{name, "%crimsonmc.plugin.incompatibleAPI"}));
                            }
                ...
                for (String name : new ArrayList<>(plugins.keySet())) {
                        ...
                        for (String dependency : new ArrayList<>(dependencies.get(name))) {
                            if (loadedPlugins.containsKey(dependency) || this.getPlugin(dependency) != null) {
                                ...
                                this.server.getLogger().critical(this.server.getLanguage().translateString(
                                        "crimsonmc"
                                                + ".plugin.loadError",
                                        new String[]{name, "%crimsonmc.plugin.unknownDependency"}));
                                break;
                            }
                        }
                    ...
                    if (!dependencies.containsKey(name) && !softDependencies.containsKey(name)) {
                        ...
                        if (plugin != null) {
                            ...
                            this.server.getLogger().critical(this.server.getLanguage().translateString(
                                    "crimsonmc.plugin.genericLoadError", name));
                        }
                    }
                }
                ...
                if (missingDependency) {
                    ...
                    if (missingDependency) {
                        for (String name : plugins.keySet()) {
                            this.server.getLogger().critical(this.server.getLanguage().translateString(
                                    "crimsonmc.plugin.loadError",
                                    new String[]{name, "%crimsonmc.plugin.circularDependency"}));
                        }
                        plugins.clear();
                    }
                }
    ...
    public void disablePlugins() {
        ListIterator<Plugin> plugins =
                new ArrayList<>(this.getPlugins().values()).listIterator(this.getPlugins().size());
        ...
        try {
            for (RegisteredListener registration :
                ...
                try {
                    ...
                    this.server.getLogger().critical(this.server.getLanguage().translateString(
                            "crimsonmc.plugin.eventError", event.getEventName(),
    ...
    public void registerEvents(Listener listener, Plugin plugin) {
        if (!plugin.isEnabled()) {
            throw new PluginException("Plugin attempted to register " + listener.getClass().getName() +
                    " while not enabled");
        }
        ...
        try {
            ...
            plugin.getLogger().error("Plugin " + plugin.getDescription().getFullName() +
        ...
        for (final Method method : methods) {
            ...
            for (Class<?> clazz = eventClass; Event.class.isAssignableFrom(clazz);
                ...
                if (clazz.getAnnotation(Deprecated.class) != null) {
                    if (Boolean.valueOf(
                            String.valueOf(this.server.getConfig("settings.deprecated-verbpse", true)))) {
                        this.server.getLogger().warning(this.server.getLanguage().translateString(
                                "crimsonmc.plugin.deprecatedEvent", plugin.getName(), clazz.getName(),
                                listener.getClass().getName() + "." + method.getName() + "()"));
                    }
                    break;
                }
            }
            this.registerEvent(eventClass, listener, eh.priority(), new MethodEventExecutor(method),
                    plugin, eh.ignoreCancelled());
        }
    }

    public void registerEvent(Class<? extends Event> event, Listener listener, EventPriority priority,
                              EventExecutor executor, Plugin plugin) throws PluginException {
    ...
    private Class<? extends Event> getRegistrationClass(Class<? extends Event> clazz)
        ...
        try {
            ...
            if (clazz.getSuperclass() != null && !clazz.getSuperclass().equals(Event.class) &&
                ...
                throw new IllegalAccessException("Unable to find handler list for event " +
                        clazz.getName() + ". Static getHandlers method required!");
            }
        }
    }
}
