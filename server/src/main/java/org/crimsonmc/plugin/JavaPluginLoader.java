...
import org.crimsonmc.event.plugin.PluginDisableEvent;
import org.crimsonmc.event.plugin.PluginEnableEvent;
import org.crimsonmc.exception.PluginException;
import org.crimsonmc.plugin.certification.PluginCertificateTask;
...
public class JavaPluginLoader implements PluginLoader {
    ...
    public Plugin loadPlugin(File file) throws Exception {
        ...
        if (description != null) {
            this.server.getLogger().info(this.server.getLanguage().translateString(
                    "crimsonmc.plugin.load", description.getFullName()));
            File dataFolder = new File(file.getParentFile(), description.getName());
            if (dataFolder.exists() && !dataFolder.isDirectory()) {
                throw new IllegalStateException("Projected dataFolder '" + dataFolder + "' for " +
            ...
            try {
                ...
                try {
                    ...
                    throw new PluginException("main class `" + description.getMain() +
                            "' does not extend PluginBase");
                } catch (InstantiationException | IllegalAccessException e) {
                    Server.getInstance().getLogger().exception(e);
                }

            } catch (ClassNotFoundException e) {
                throw new PluginException("Couldn't load plugin " + description.getName() +
    ...
    public PluginDescription getPluginDescription(File file) {
        try {
            ...
            JarEntry entry = jar.getJarEntry("crimsonmc.yml");
            if (entry == null) {
    ...
    private void initPlugin(PluginBase plugin, PluginDescription description, File dataFolder,
                            File file) {
    ...
    public void enablePlugin(Plugin plugin) {
        if (plugin instanceof PluginBase && !plugin.isEnabled()) {
            this.server.getLogger().info(this.server.getLanguage().translateString(
                    "crimsonmc.plugin.enable", plugin.getDescription().getFullName()));
    ...
    public void disablePlugin(Plugin plugin) {
        if (plugin instanceof PluginBase && plugin.isEnabled()) {
            this.server.getLogger().info(this.server.getLanguage().translateString(
                    "crimsonmc.plugin.disable", plugin.getDescription().getFullName()));
