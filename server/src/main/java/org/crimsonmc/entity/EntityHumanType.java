...
import org.crimsonmc.inventory.PlayerInventory;
...
public abstract class EntityHumanType extends EntityCreature implements InventoryHolder {
            ...
            for (CompoundTag item : inventoryList.getAll()) {
                ...
                if (slot >= 0 && slot < 9) {
                    this.inventory.setHotbarSlotIndex(
                            slot, item.contains("TrueSlot") ? item.getByte("TrueSlot") : -1);
    ...
    public void saveNBT() {
        ...
        if (this.inventory != null) {
            for (int slot = 0; slot < 9; ++slot) {
                ...
                if (hotbarSlot != -1) {
                    ...
                    if (item.getId() != 0 && item.getCount() > 0) {
                        this.namedTag.getList("Inventory", CompoundTag.class)
                                .add(NBTIO.putItemHelper(item, slot).putByte("TrueSlot", hotbarSlot));
                        continue;
                    }
                }
                this.namedTag.getList("Inventory", CompoundTag.class)
            ...
            int slotCount = ServerPlayer.SURVIVAL_SLOTS + 9;
    ...
    public void attack(EntityDamageEvent source) {
        ...
        if (source.getCause() != EntityDamageEvent.CAUSE_VOID &&
                source.getCause() != EntityDamageEvent.CAUSE_CUSTOM &&
            ...
            float r = (source.getDamage(EntityDamageEvent.MODIFIER_ARMOR) -
                    (originalDamage -
                            originalDamage * (1 - Math.max(points / 5, points - originalDamage / 2) / 25)));
            ...
            source.setDamage(source.getDamage(EntityDamageEvent.MODIFIER_ARMOR_ENCHANTMENTS) -
                            (originalDamage - originalDamage * (1 - epf / 25)),
                    EntityDamageEvent.MODIFIER_ARMOR_ENCHANTMENTS);
        }
        ...
        if (!source.isCancelled()) {
            if (source instanceof EntityDamageByEntityEvent) {
                ...
                if (thornsDamage > 0) {
                    damager.attack(
                            new EntityDamageEvent(damager, EntityDamageEvent.CAUSE_MAGIC, rnd.nextInt(4) + 1));
                }
            }
