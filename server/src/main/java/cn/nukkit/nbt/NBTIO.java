...
public class NBTIO {
   ...
   */
  public static CompoundTag putItemHelper(Item item) { return putItemHelper(item, null); }
  ...
  public static Item getItemHelper(CompoundTag tag) {
    ...
    Item item = Item.get(tag.getShort("id"), !tag.contains("Damage") ? 0 : tag.getShort("Damage"),
                         tag.getByte("Count"));
  ...
  public static CompoundTag readCompressed(byte[] data, ByteOrder endianness) throws IOException {
    return read(new BufferedInputStream(new GZIPInputStream(new ByteArrayInputStream(data))),
                endianness);
  }
  ...
  public static void writeGZIPCompressed(CompoundTag tag, OutputStream outputStream,
                                         ByteOrder endianness) throws IOException {
