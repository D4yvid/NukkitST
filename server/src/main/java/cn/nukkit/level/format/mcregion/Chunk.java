...
public class Chunk extends BaseFullChunk {
  ...
  public Chunk(LevelProvider level) { this(level, null); }
  ...
  public Chunk(LevelProvider level, CompoundTag nbt) {
    ...
    if (!(this.nbt.contains("BiomeColors") &&
          (this.nbt.get("BiomeColors") instanceof IntArrayTag))) {
  ...
  public static Chunk fromBinary(byte[] data, LevelProvider provider) {
    try {
      CompoundTag chunk =
          NBTIO.read(new ByteArrayInputStream(Zlib.inflate(data)), ByteOrder.BIG_ENDIAN);

      if (!chunk.contains("Level") || !(chunk.get("Level") instanceof CompoundTag)) {
        return null;
      }
      return new Chunk(provider != null ? provider : McRegion.class.newInstance(),
                       chunk.getCompound("Level"));
    } catch (Exception e) {
      return null;
    }
  }
  ...
  public byte[] toFastBinary() {
    ...
    stream.putByte((byte)((this.isLightPopulated() ? 1 << 2 : 0) +
                          (this.isPopulated() ? 1 << 2 : 0) + (this.isGenerated() ? 1 : 0)));
