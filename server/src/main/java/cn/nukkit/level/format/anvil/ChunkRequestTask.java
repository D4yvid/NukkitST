...
public class ChunkRequestTask extends AsyncTask {
      ...
      if (blockEntity instanceof BlockEntitySpawnable) {
        try {
          buffer = Binary.appendBytes(
              buffer, NBTIO.write(((BlockEntitySpawnable)blockEntity).getSpawnCompound(),
  ...
  public void onRun() {
    ...
    ByteBuffer buffer =
        ByteBuffer.allocate(16 * 16 * (128 + 64 + 64 + 64) + 256 + 256 + this.blockEntities.length);
    ...
    this.setResult(buffer.put(orderedIds)
                       .put(orderedData)
                       .put(orderedSkyLight)
                       .put(orderedLight)
  ...
  public byte[] getHalfColumn(byte[] data, int x, int z) {
    ...
    if ((x & 1) == 0) {
      for (int y = 0; y < 128; y += 2) {
        column[(y / 2)] = (byte)((byte)(data[(y << 7) + i] & 0x0f) |
                                 (byte)(((data[((y + 1) << 7) + i] & 0x0f) & 0xff) << 4));
      }
    } else {
      for (int y = 0; y < 128; y += 2) {
        column[(y / 2)] = (byte)((byte)(((data[(y << 7) + i] & 0xf0) & 0xff) >> 4) |
                                 (byte)(data[((y + 1) << 7) + i] & 0xf0));
      }
    }
    return column;
  }
