...
public abstract class BaseLevelProvider implements LevelProvider {
  ...
  public BaseLevelProvider(Level level, String path) throws IOException {
    ...
    CompoundTag levelData = NBTIO.readCompressed(
        new FileInputStream(new File(this.getPath() + "level.dat")), ByteOrder.BIG_ENDIAN);
  ...
  public Server getServer() { return this.level.getServer(); }
  ...
  public CompoundTag getLevelData() { return levelData; }

  @Override
  public void saveLevelData() {
    try {
      NBTIO.writeGZIPCompressed(new CompoundTag().putCompound("Data", this.levelData),
                                new FileOutputStream(this.getPath() + "level.dat"));
    } catch (IOException e) {
