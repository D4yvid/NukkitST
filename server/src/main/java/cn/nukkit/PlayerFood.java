...
public class PlayerFood {
  ...
  public int getLevel() { return this.foodLevel; }

  public void setLevel(int foodLevel) { this.setLevel(foodLevel, -1); }

  public int getMaxLevel() { return this.maxFoodLevel; }
  ...
  public void setLevel(int foodLevel, float saturationLevel) {
    ...
    PlayerFoodLevelChangeEvent ev =
        new PlayerFoodLevelChangeEvent(this.getPlayer(), foodLevel, saturationLevel);
  ...
  public float getFoodSaturationLevel() { return this.foodSaturationLevel; }
  ...
  public void setFoodSaturationLevel(float fsl) {
    ...
    if (fsl < 0)
      fsl = 0;
    PlayerFoodLevelChangeEvent ev =
        new PlayerFoodLevelChangeEvent(this.getPlayer(), this.getLevel(), fsl);
    this.getPlayer().getServer().getPluginManager().callEvent(ev);
    if (ev.isCancelled()) {
      return;
    }
  ...
  public void useHunger() { this.useHunger(1); }
  ...
  public void sendFoodLevel() { this.sendFoodLevel(this.getLevel()); }
  ...
  public void sendFoodLevel(int foodLevel) {
    if (this.getPlayer().isSpawned()) {
      this.getPlayer().setAttribute(
          Attribute.getAttribute(Attribute.MAX_HUNGER).setValue(foodLevel));
    }
  }
    ...
    if (this.getLevel() > 17) {
      ...
      if (this.foodTickTimer >= 80) {
        if (this.getPlayer().getHealth() < this.getPlayer().getMaxHealth()) {
          EntityRegainHealthEvent ev = new EntityRegainHealthEvent(
              this.getPlayer(), 1, EntityRegainHealthEvent.CAUSE_EATING);
      ...
      if (this.foodTickTimer >= 80) {
        EntityDamageEvent ev =
            new EntityDamageEvent(this.getPlayer(), EntityDamageEvent.CAUSE_VOID, 1);
  ...
  public void updateFoodExpLevel(double use) {
    ...
    if (Server.getInstance().getDifficulty() == 0)
      return;
    if (this.getPlayer().hasEffect(Effect.SATURATION))
      return;
