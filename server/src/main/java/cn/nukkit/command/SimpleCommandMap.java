...
public class SimpleCommandMap implements CommandMap {
  ...
  public boolean dispatch(CommandSender sender, String cmdLine) {
    ...
    try {
      ...
      this.server.getLogger().critical(this.server.getLanguage().translateString(
          "nukkit.command.exception", cmdLine, target.toString(), Utils.getExceptionMessage(e)));
  ...
  public Map<String, Command> getCommands() { return knownCommands; }
    ...
    for (Map.Entry<String, List<String>> entry : values.entrySet()) {
      ...
      if (alias.contains(" ") || alias.contains(":")) {
        this.server.getLogger().warning(
            this.server.getLanguage().translateString("nukkit.command.alias.illegal", alias));
        continue;
      }
      ...
      if (bad.length() > 0) {
        this.server.getLogger().warning(this.server.getLanguage().translateString(
            "nukkit.command.alias.notFound", new String[] {alias, bad}));
        continue;
      }
