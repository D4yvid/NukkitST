 ...
 * api}这几个字段是必需的， 要让Nukkit能够正常加载你的插件，必须要合理地填写这几个字段。<br> When
 ...
 * sure your plugin can be normally loaded by Nukkit.<br>
 * </p>
 * <p>
 * <p>接下来对所有的字段做一些说明，<b>加粗</b>的字段表示必需，<i>斜体</i>表示可选：（来自
 * <a
 * href="http://www.cnblogs.com/xtypr/p/nukkit_plugin_start_from_0_about_config.html">粉鞋大妈的博客文章</a>）<br>
 * Here are some instructions for there items, <b>bold</b> means required, <i>italic</i> means
 * optional: (From <a
 * href="http://www.cnblogs.com/xtypr/p/nukkit_plugin_start_from_0_about_config.html">a blog article
 ...
 * A plugin name <i>can't contain 'nukkit' 'minecraft' 'mojang'</i>, and shouldn't contain
 ...
 * <li><b>api</b><br>
 * 字符串序列，表示这个插件支持的Nukkit
 * API版本号列表。插件作者应该调试能支持的API，然后把版本号添加到这个列表。<br> A set of String, the
 * Nukkit API versions that the plugin supports. Plugin developers should debug in different Nukkit
 ...
 * String, the main class of plugin. The main class<i> can't be placed at 'cn.nukkit'
...
public class PluginDescription {
  ...
  public PluginDescription(Map<String, Object> yamlMap) { this.loadMap(yamlMap); }
  ...
  private void loadMap(Map<String, Object> plugin) throws PluginException {
    ...
    if (this.main.startsWith("cn.nukkit.")) {
      throw new PluginException(
          "Invalid PluginDescription main, cannot start within the cn.nukkit. package");
    }
   ...
   * @since Nukkit 1.0 | Nukkit API 1.0.0
   */
  public String getFullName() { return this.name + " v" + this.version; }

  /**
   * 返回这个插件支持的Nukkit API版本列表。<br> Returns all Nukkit API versions this plugin
   * supports.
   *
   * @return 这个插件支持的Nukkit API版本列表。<br>A list of all Nukkit API versions String this
   *     plugin supports.
   * @see PluginDescription
   * @since Nukkit 1.0 | Nukkit API 1.0.0
   */
  public List<String> getCompatibleAPIs() { return api; }
   ...
   * Nukkit won't load this plugin.</li>
   * <li>当一个插件所依赖的插件不存在时，Nukkit不会加载这个插件，但是会提醒用户去安装所依赖的插件。<br>
   * When the required dependency plugin does not exists, Nukkit won't load this plugin, but will
   ...
   * installing B, Nukkit won't load A because its dependency B is lost. Only when B is installed, A
   * will be loaded by Nukkit.</p>
   *
   ...
   * The load action of a Nukkit plugin begins from main class. The name of main class should be
   ...
   * @since Nukkit 1.0 | Nukkit API 1.0.0
   */
  public PluginLoadOrder getOrder() { return order; }
