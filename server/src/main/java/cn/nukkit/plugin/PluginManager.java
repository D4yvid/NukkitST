...
public class PluginManager {
  ...
  public Plugin loadPlugin(String path) { return this.loadPlugin(path, null); }

  public Plugin loadPlugin(File file) { return this.loadPlugin(file, null); }
        ...
        for (File file : dictionary.listFiles(new FilenameFilter() {
          ...
          try {
            ...
            if (description != null) {
              ...
              if (name.toLowerCase().contains("nukkit") ||
              ...
              for (String version : description.getCompatibleAPIs()) {
                ...
                if (!Pattern.matches("[0-9]\\.[0-9]\\.[0-9]", version)) {
                  this.server.getLogger().error(this.server.getLanguage().translateString(
                      "nukkit.plugin.loadError", new String[] {name, "Wrong API format"}));
                  continue;
                }
              ...
              if (!compatible) {
                this.server.getLogger().error(this.server.getLanguage().translateString(
                    "nukkit.plugin.loadError",
                    new String[] {name, "%nukkit.plugin.incompatibleAPI"}));
              }
        ...
        for (String name : new ArrayList<>(plugins.keySet())) {
          ...
          if (!dependencies.containsKey(name) && !softDependencies.containsKey(name)) {
            ...
            if (plugin != null) {
              ...
              this.server.getLogger().critical(this.server.getLanguage().translateString(
                  "nukkit.plugin.genericLoadError", name));
            }
          }
        }
  ...
  public void removePermission(String name) { this.permissions.remove(name); }
    ...
    try {
      for (RegisteredListener registration :
        ...
        try {
          ...
          this.server.getLogger().critical(this.server.getLanguage().translateString(
              "nukkit.plugin.eventError", event.getEventName(),
      ...
      for (Class<?> clazz = eventClass; Event.class.isAssignableFrom(clazz);
        ...
        if (clazz.getAnnotation(Deprecated.class) != null) {
          if (Boolean.valueOf(
            ...
            this.server.getLogger().warning(this.server.getLanguage().translateString(
                "nukkit.plugin.deprecatedEvent", plugin.getName(), clazz.getName(),
