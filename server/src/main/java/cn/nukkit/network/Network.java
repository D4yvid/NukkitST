...
public class Network {
  ...
  public static final byte CHANNEL_PRIORITY = 1; // Priority channel, only to be used when it
                                                 // matters
  ...
  public Set<SourceInterface> getInterfaces() { return interfaces; }
    ...
    for (SourceInterface interfaz : this.interfaces) {
      try {
        ...
        this.server.getLogger().critical(this.server.getLanguage().translateString(
            ...
            new String[] {interfaz.getClass().getName(), e.getMessage()}));
      }
    }
  }
