...
public class Attribute implements Cloneable {
  ...
  private Attribute(int id, String name, float minValue, float maxValue, float defaultValue,
                    boolean shouldSend) {
    this.id = id;
    this.name = name;
  ...
  public static void init() {
    addAttribute(ABSORPTION, "generic.absorption", 0.00f,
                 340282346638528859811704183484516925440.00f, 0.00f);
    ...
    addAttribute(MOVEMENT_SPEED, "generic.movementSpeed", 0.00f,
                 340282346638528859811704183484516925440.00f, 0.10f);
    ...
    addAttribute(ATTACK_DAMAGE, "generic.attackDamage", 0.00f,
                 340282346638528859811704183484516925440.00f, 1.00f, false);
  ...
  public static Attribute addAttribute(int id, String name, float minValue, float maxValue,
                                       float defaultValue) {
    return addAttribute(id, name, minValue, maxValue, defaultValue, true);
  }

  public static Attribute addAttribute(int id, String name, float minValue, float maxValue,
                                       float defaultValue, boolean shouldSend) {
    if (minValue > maxValue || defaultValue > maxValue || defaultValue < minValue) {
      throw new IllegalArgumentException("Invalid ranges: min value: " + minValue +
                                         ", max value: " + maxValue +
                                         ", defaultValue: " + defaultValue);
    }

    return attributes.put(id,
                          new Attribute(id, name, minValue, maxValue, defaultValue, shouldSend));
  }
  ...
  public Attribute setValue(float value) { return setValue(value, false); }
  ...
  public boolean isSyncable() { return this.shouldSend; }
