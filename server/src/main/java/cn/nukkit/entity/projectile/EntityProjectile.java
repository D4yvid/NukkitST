...
public abstract class EntityProjectile extends Entity {
  ...
  public EntityProjectile(FullChunk chunk, CompoundTag nbt) { this(chunk, nbt, null); }
  ...
  protected double getDamage() { return 0; }
  ...
  public boolean onUpdate(int currentTick) {
    ...
    if (this.isAlive()) {
      ...
      Entity[] list = this.getLevel().getCollidingEntities(
      ...
      if (movingObjectPosition != null) {
        if (movingObjectPosition.entityHit != null) {
          ...
          this.server.getPluginManager().callEvent(
              hitEvent = new ProjectileHitEvent(this, movingObjectPosition));

          if (!hitEvent.isCancelled()) {
            movingObjectPosition = hitEvent.getMovingObjectPosition();
            double motion = Math.sqrt(this.motionX * this.motionX + this.motionY * this.motionY +
            ...
            if (this.shootingEntity == null) {
              ev = new EntityDamageByEntityEvent(this, movingObjectPosition.entityHit,
                                                 EntityDamageEvent.CAUSE_PROJECTILE, (float)damage);
            } else {
              ev = new EntityDamageByChildEntityEvent(
                  this.shootingEntity, this, movingObjectPosition.entityHit,
                  EntityDamageEvent.CAUSE_PROJECTILE, (float)damage);
            }
            ...
            if (this.fireTicks > 0) {
              EntityCombustByEntityEvent ev2 =
      ...
      if (this.isCollided && !this.hadCollision) {
        ...
        this.server.getPluginManager().callEvent(new ProjectileHitEvent(
            this, MovingObjectPosition.fromBlock(this.getFloorX(), this.getFloorY(),
                                                 this.getFloorZ(), -1, this)));
      } else if (!this.isCollided && this.hadCollision) {
        this.hadCollision = false;
      }

      if (!this.onGround || Math.abs(this.motionX) > 0.00001 || Math.abs(this.motionY) > 0.00001 ||
