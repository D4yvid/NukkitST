...
public class MainLogger extends ThreadedLogger {
  ...
  public MainLogger(String logFile) { this(logFile, false); }
  ...
  public MainLogger(String logFile, Boolean logDebug) {
    ...
    replacements.put(TextFormat.BLACK,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.BLACK).boldOff().toString());
    replacements.put(TextFormat.DARK_BLUE,
    ...
    replacements.put(TextFormat.DARK_AQUA,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.CYAN).boldOff().toString());
    replacements.put(TextFormat.DARK_RED,
    ...
    replacements.put(TextFormat.GOLD,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.YELLOW).boldOff().toString());
    replacements.put(TextFormat.GRAY,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.WHITE).boldOff().toString());
    replacements.put(TextFormat.DARK_GRAY,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.BLACK).bold().toString());
    replacements.put(TextFormat.BLUE,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.BLUE).bold().toString());
    replacements.put(TextFormat.GREEN,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.GREEN).bold().toString());
    replacements.put(TextFormat.AQUA,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.CYAN).bold().toString());
    replacements.put(TextFormat.RED,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.RED).bold().toString());
    replacements.put(TextFormat.LIGHT_PURPLE,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.MAGENTA).bold().toString());
    replacements.put(TextFormat.YELLOW,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.YELLOW).bold().toString());
    replacements.put(TextFormat.WHITE,
                     Ansi.ansi().a(Attribute.RESET).fg(Color.WHITE).bold().toString());
    replacements.put(TextFormat.BOLD, Ansi.ansi().a(Attribute.UNDERLINE_DOUBLE).toString());
    replacements.put(TextFormat.STRIKETHROUGH,
  ...
  public static MainLogger getLogger() { return logger; }
  ...
  public void setLogDebug(Boolean logDebug) { this.logDebug = logDebug; }

  public void logException(Exception e) { this.alert(Utils.getExceptionMessage(e)); }
  ...
  protected void send(String message) { this.send(message, -1); }

  protected void send(String message, int level) {
    Date now = new Date();
    message = TextFormat.AQUA + new SimpleDateFormat("HH:mm:ss").format(now) + TextFormat.RESET +
              " " + message + TextFormat.RESET;

    System.out.println(colorize(message));

    String str =
        new SimpleDateFormat("Y-M-d").format(now) + " " + TextFormat.clean(message) + "\r\n";
    this.logStream += str;
  ...
  public void run() {
      ...
      while (this.logStream.length() > 0) {
        ...
        try {
          OutputStreamWriter writer = new OutputStreamWriter(
              new FileOutputStream(this.logFile, true), StandardCharsets.UTF_8);
          writer.write(chunk);
          writer.flush();
          writer.close();
        } catch (Exception e) {
    ...
    if (this.logStream.length() > 0) {
      ...
      this.logStream = "";
      try {
        OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(this.logFile, true),
