...
/**
 * 用来描述一个玩家和获得这个玩家相应信息的接口。<br> An interface to describe a player and get its
 * information. <p> <p>这个玩家可以在线，也可以是不在线。<br> This player can be online or
 * offline.</p>
 *
...
public interface IPlayer extends ServerOperator, Metadatable {
   ...
   * cn.nukkit.Player#getDisplayName}<br> Notice that this will only return its login name. If you
   * need its display name, turn to {@link cn.nukkit.Player#getDisplayName}</p>
   *
