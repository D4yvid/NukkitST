...
public class BlockDoublePlant extends BlockFlowable {
  ...
  public BlockDoublePlant() { this(0); }

  public BlockDoublePlant(int meta) { super(meta); }
  ...
  public String getName() {
    String[] names =
        new String[] {"Sunflower", "Lilac", "Double Tallgrass", "Large Fern", "Rose Bush", "Peony"};
    ...
    if (type == Level.BLOCK_UPDATE_NORMAL) {
      if ((this.meta & 0x08) == 8) {
        ...
        if (!(this.getSide(0).getId() == DOUBLE_PLANT)) {
          this.getLevel().useBreakOn(this); // Using useBreakOn.getSide(Vector3.SIDE_DOWN) doesn't
  ...
  public boolean place(Item item, Block block, Block target, int face, double fx, double fy,
    ...
    if (up.getId() == 0 && (down.getId() == GRASS || down.getId() == DIRT)) {
      this.getLevel().setBlock(block, this, true,
                               false); // If we update the bottom half, it will drop the item
                                       // because there isn't a flower block above
