...
public class BlockAnvil extends BlockFallable {
  ...
  public BlockAnvil() { this(0); }

  public BlockAnvil(int meta) { super(meta); }
  ...
  public int[][] getDrops(Item item) {
    ...
    if (item.isPickaxe() && item.getTier() >= ItemTool.TIER_WOODEN) {
      if (damage >= 0 && damage <= 3) { // Anvil
        ...
        return new int[][] {{this.getId(), this.meta & 0x04, 1}};
      } else if (damage >= 8 && damage <= 11) { // Very Damaged Anvil
        return new int[][] {{this.getId(), this.meta & 0x08, 1}};
      }
    }
    return new int[0][0];
  }
