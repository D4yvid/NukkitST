...
public class BlockSlabWood extends BlockSlab {
  ...
  public BlockSlabWood() { this(0); }

  public BlockSlabWood(int meta) { super(meta); }

  @Override
  public String getName() {
    String[] names =
        new String[] {"Oak", "Spruce", "Birch", "Jungle", "Acacia", "Dark Oak", "", ""};
    return (((this.meta & 0x08) == 0x08) ? "Upper " : "") + names[this.meta & 0x07] +
        " Wooden Slab";
  }
  ...
  public boolean place(Item item, Block block, Block target, int face, double fx, double fy,
    ...
    this.meta &= 0x07;
    if (face == 0) {
      if (target.getId() == WOOD_SLAB && (target.getDamage() & 0x08) == 0x08 &&
          (target.getDamage() & 0x07) == (this.meta & 0x07)) {
      ...
      if (target.getId() == WOOD_SLAB && (target.getDamage() & 0x08) == 0 &&
          (target.getDamage() & 0x07) == (this.meta & 0x07)) {
        this.getLevel().setBlock(target, new BlockDoubleSlabWood(this.meta), true);
  ...
  public int[][] getDrops(Item item) {
    return new int[][] {{this.getId(), this.meta & 0x07, 1}};
  }
