...
public class BlockSlab extends BlockTransparent {
  ...
  public BlockSlab() { this(0); }

  public BlockSlab(int meta) { super(meta); }
  ...
  public String getName() {
    String[] names = new String[] {"Stone", "Sandstone",   "Wooden", "Cobblestone",
  ...
  public boolean place(Item item, Block block, Block target, int face, double fx, double fy,
    ...
    this.meta &= 0x07;
    if (face == 0) {
      if (target.getId() == SLAB && (target.getDamage() & 0x08) == 0x08 &&
      ...
      if (target.getId() == SLAB && (target.getDamage() & 0x08) == 0 &&
          (target.getDamage() & 0x07) == (this.meta & 0x07)) {
        this.getLevel().setBlock(target, new BlockDoubleSlab(this.meta), true);
