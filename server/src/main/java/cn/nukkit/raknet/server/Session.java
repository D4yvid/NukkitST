...
public class Session {
  ...
  public void disconnect() throws Exception { this.disconnect("unknown"); }
  ...
  public void addEncapsulatedToQueue(EncapsulatedPacket packet, int flags) throws Exception {
    ...
    if (packet.reliability == 2 || packet.reliability == 3 || packet.reliability == 4 ||
        packet.reliability == 6 || packet.reliability == 7) {
  ...
  private void handleSplit(EncapsulatedPacket packet) throws Exception {
    if (packet.splitCount >= MAX_SPLIT_SIZE || packet.splitIndex >= MAX_SPLIT_SIZE ||
        packet.splitIndex < 0) {
      return;
    }
  ...
  private void handleEncapsulatedPacketRoute(EncapsulatedPacket packet) throws Exception {
    ...
    if ((id & 0xff) < 0x80) { // internal data packet
      if (state == STATE_CONNECTING_2) {
        if (id == CLIENT_CONNECT_DataPacket.ID) {
          ...
          if (dataPacket.port == this.sessionManager.getPort() ||
              !this.sessionManager.portChecking) {
      ...
      // this.sessionManager.getLogger().notice("Received packet before connection:
  ...
  public void handlePacket(Packet packet) throws Exception {
    ...
    if (this.state == STATE_CONNECTED || this.state == STATE_CONNECTING_2) {
      if (((packet.buffer[0] & 0xff) >= 0x80 || (packet.buffer[0] & 0xff) <= 0x8f) &&
          packet instanceof DataPacket dp) {
        dp.decode();

        if (dp.seqNumber < this.windowStart || dp.seqNumber > this.windowEnd ||
            this.receivedWindow.containsKey(dp.seqNumber)) {
          return;
        }
            ...
            if (this.recoveryQueue.containsKey(seq)) {
              for (Object pk : this.recoveryQueue.get(seq).packets) {
                if (pk instanceof EncapsulatedPacket && ((EncapsulatedPacket)pk).needACK &&
                    ((EncapsulatedPacket)pk).messageIndex != null) {
                  if (this.needACK.containsKey(((EncapsulatedPacket)pk).identifierACK)) {
                    this.needACK.get(((EncapsulatedPacket)pk).identifierACK)
                        .remove(((EncapsulatedPacket)pk).messageIndex);
                  }
                }
              }
              this.recoveryQueue.remove(seq);
    ...
    } else if ((packet.buffer[0] & 0xff) > 0x00 ||
               (packet.buffer[0] & 0xff) < 0x80) { // Not Data packet :)
      packet.decode();
      if (packet instanceof OPEN_CONNECTION_REQUEST_1) {
        ...
        if (((OPEN_CONNECTION_REQUEST_2)packet).serverPort == this.sessionManager.getPort() ||
            !this.sessionManager.portChecking) {
          this.mtuSize =
              Math.min(Math.abs(((OPEN_CONNECTION_REQUEST_2)packet).mtuSize),
                       1464); // Max size, do not allow creating large buffers to fill server memory
