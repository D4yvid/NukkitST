...
public class DoubleChestInventory extends ContainerInventory implements InventoryHolder {
  ...
  public Item getItem(int index) {
    return index < this.left.getSize() ? this.left.getItem(index)
                                       : this.right.getItem(index - this.right.getSize());
  }

  @Override
  public boolean setItem(int index, Item item) {
    return index < this.left.getSize() ? this.left.setItem(index, item)
                                       : this.right.setItem(index - this.right.getSize(), item);
  }

  @Override
  public boolean clear(int index) {
    return index < this.left.getSize() ? this.left.clear(index)
                                       : this.right.clear(index - this.right.getSize());
  }
  ...
  public void onOpen(Player who) {
    ...
    if (this.getViewers().size() == 1) {
      ...
      if (level != null) {
        level.addChunkPacket((int)this.left.getHolder().getX() >> 4,
                             (int)this.left.getHolder().getZ() >> 4, pk1);
      }
      ...
      if (level != null) {
        level.addChunkPacket((int)this.right.getHolder().getX() >> 4,
                             (int)this.right.getHolder().getZ() >> 4, pk2);
      }
    }
  }
  ...
  public void onClose(Player who) {
    if (this.getViewers().size() == 1) {
      ...
      if (level != null) {
        level.addChunkPacket((int)this.right.getHolder().getX() >> 4,
                             (int)this.right.getHolder().getZ() >> 4, pk1);
      }
      ...
      if (level != null) {
        level.addChunkPacket((int)this.left.getHolder().getX() >> 4,
                             (int)this.left.getHolder().getZ() >> 4, pk2);
      }
    }
  ...
  public ChestInventory getLeftSide() { return this.left; }

  public ChestInventory getRightSide() { return this.right; }
}
