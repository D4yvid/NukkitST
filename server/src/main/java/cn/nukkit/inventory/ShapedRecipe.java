...
public class ShapedRecipe implements Recipe {
  ...
  public ShapedRecipe(Item result, String... shape) {
    ...
    if (shape.length > 3) {
      throw new IllegalStateException("Crafting recipes should be 1, 2, 3 rows, not " +
                                      shape.length);
    }
    ...
    for (int y = 0; y < shape.length; y++) {
      ...
      if (row.length() == 0 || row.length() > 3) {
        throw new IllegalStateException("Crafting rows should be 1, 2, 3 characters, not " +
                                        row.length());
      }
  ...
  public int getWidth() { return this.ingredients.get(0).size(); }

  public int getHeight() { return this.ingredients.size(); }
  ...
  public Item getIngredient(int x, int y) {
    if (this.ingredients.containsKey(y)) {
      if (this.ingredients.get(y).containsKey(x)) {
        return this.ingredients.get(y).get(x) != null ? this.ingredients.get(y).get(x)
                                                      : Item.get(Item.AIR);
      }
    }
  ...
  public Map<Character, String> getShape() { return shapes; }
