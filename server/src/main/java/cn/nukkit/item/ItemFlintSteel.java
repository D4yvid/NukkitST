...
public class ItemFlintSteel extends ItemTool {
  ...
  public ItemFlintSteel() { this(0, 1); }

  public ItemFlintSteel(Integer meta) { this(meta, 1); }
  ...
  public boolean onActivate(Level level, Player player, Block block, Block target, int face,
    ...
    if (block.getId() == AIR && (target instanceof BlockSolid)) {
      if (target.getId() == OBSIDIAN) {
        ...
        int x_min = targetX;
        int x;
        for (x = targetX + 1; level.getBlock(new Vector3(x, targetY, targetZ)).getId() == OBSIDIAN;
             x++) {
          x_max++;
        }
        for (x = targetX - 1; level.getBlock(new Vector3(x, targetY, targetZ)).getId() == OBSIDIAN;
        ...
        int z_min = targetZ;
        int z;
        for (z = targetZ + 1; level.getBlock(new Vector3(targetX, targetY, z)).getId() == OBSIDIAN;
             z++) {
          z_max++;
        }
        for (z = targetZ - 1; level.getBlock(new Vector3(targetX, targetY, z)).getId() == OBSIDIAN;
        ...
        if ((count_x >= 4 && count_x <= 23 || count_z >= 4 && count_z <= 23) && count_y >= 5 &&
            count_y <= 23) {
          int count_up = 0;
          for (int up_z = z_min;
               level.getBlock(new Vector3(targetX, y_max, up_z)).getId() == OBSIDIAN &&
               up_z <= z_max;
               up_z++) {
            count_up++;
          }
      ...
      if (fire.isBlockTopFacingSurfaceSolid(fire.getSide(Vector3.SIDE_DOWN)) ||
        ...
        BlockIgniteEvent e = new BlockIgniteEvent(
            block, null, player, BlockIgniteEvent.BlockIgniteCause.FLINT_AND_STEEL);
