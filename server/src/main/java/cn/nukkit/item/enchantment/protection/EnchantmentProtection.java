...
public abstract class EnchantmentProtection extends Enchantment {
  ...
  protected EnchantmentProtection(int id, String name, int weight,
  ...
  public int getDamageProtection(EntityDamageEvent event) {
    ...
    if (level <= 0 || cause == EntityDamageEvent.CAUSE_VOID ||
        cause == EntityDamageEvent.CAUSE_CUSTOM || cause == EntityDamageEvent.CAUSE_MAGIC) {
      return 0;
    }
  ...
  public double getTypeModifier() { return 0; }
  ...
  public enum TYPE { ALL, FIRE, FALL, EXPLOSION, PROJECTILE }
}
