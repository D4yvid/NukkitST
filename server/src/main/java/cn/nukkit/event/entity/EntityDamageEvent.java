...
public class EntityDamageEvent extends EntityEvent implements Cancellable {
  ...
  public EntityDamageEvent(Entity entity, int cause, Map<Integer, Float> modifiers) {
    ...
    if (entity.hasEffect(Effect.DAMAGE_RESISTANCE)) {
      this.setDamage((float)-(this.getDamage(MODIFIER_BASE) * 0.20 *
                              (entity.getEffect(Effect.DAMAGE_RESISTANCE).getAmplifier() + 1)),
                     MODIFIER_RESISTANCE);
    }
  }
  ...
  public float getOriginalDamage() { return this.getOriginalDamage(MODIFIER_BASE); }
  ...
  public float getDamage() { return this.getDamage(MODIFIER_BASE); }

  public void setDamage(float damage) { this.setDamage(damage, MODIFIER_BASE); }
  ...
  public void setDamage(float damage, int type) { this.modifiers.put(type, damage); }

  public boolean isApplicable(int type) { return this.modifiers.containsKey(type); }
